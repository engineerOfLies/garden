  ...
  for(j = sy; (j < sh + sy)&&(j < GameMap.map.h);j++)
  {
    for(i = sx; (i < sw + sx)&&(i < GameMap.map.w);i++)
    {
      if((EntityGroundList[j][i] != NULL)&&(EntityGroundList[j][i] != &DEADSPACE))
      {
       ...
       {
//        if(GameMap.playerviews[ThisPlayer].seen[j/2][i/2])
  //      {
           if(EntityGroundList[j][i]->LastDrawn != NOW)
           {
            if(GameMap.playerviews[ThisPlayer].seeing[j][i])/*||(EntityGroundList[j][i]->Unit_Type > ET_Unit))*/
            {
              if(EntityGroundList[j][i]->shown)
                DrawEntity(EntityGroundList[j][i]);
              else if(ThisPlayer == EntityGroundList[j][i]->Player)
              {
                SDL_SetAlpha(EntityGroundList[j][i]->sprite->image,SDL_SRCALPHA,128);
                DrawEntity(EntityGroundList[j][i]);
                SDL_SetAlpha(EntityGroundList[j][i]->sprite->image,0,0);
              }
            }
...
int MoveUnit(Entity *self)
{
 ...
 if(((yposition >= 0)&&(xposition >= 0)&&(yposition < GameMap.map.h)&&(xposition < GameMap.map.w))&&((EntityGroundList[yposition][xposition] == NULL)||(EntityGroundList[yposition][xposition] == self)))
 {
  ...
  {
    EntityGroundList[self->m.y][self->m.x] = NULL;
    self->m.x = xposition;
    self->m.y = yposition;
    EntityGroundList[self->m.y][self->m.x] = self;
  }
  return 1;
 }
// self->state = ST_IDLE;
